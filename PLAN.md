...
- **Fixed 2026-08-16**: the observed tiling/mirroring turned out to be caused by Pollinations' `flux` model specifically (not the 1080-vs-1024 resolution mismatch, which was the first — wrong — hypothesis). Switched to `model=turbo`, confirmed clean on both simple and complex prompts via direct comparison (see PROGRESS.md).
