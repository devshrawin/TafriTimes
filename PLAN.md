...
Per daily cycle:
- **Topic selection** (`config/topics.yaml`): rotated India-relevant satire beats (politics, cricket, Bollywood, bureaucracy, festivals, urban life), deliberately *not* pulled from live breaking news — avoids satirizing a real, currently-newsworthy identifiable person. Selection is weighted by the engagement-log (see Feedback Loop) once it has data.
...
### 3. Image: templated parody-news-graphic (no AI image generation needed)

Build a small set of reusable **fictional** graphic templates — a breaking-news-style banner, a fake quote/screenshot card, a mock headline chyron — rendered programmatically with the winning headline/quote baked in as text, using **Satori + resvg** (pure JS/WASM, portable in GitHub Actions, no native-canvas build headaches). This:
- Needs no second AI vendor/API key (Anthropic has no image API) and no per-image generation cost.
- Carries no deepfake/real-person-likeness risk, since the template depicts no photorealistic people.
- Must use its **own clearly fictional masthead/branding** ("IndianOnion" or similar) — must not copy a real news channel's actual logo/on-air branding, to avoid trademark issues and to avoid *increasing* the "mistaken for real news" risk The Fauxy has already run into. Bake a small, permanent "SATIRE" mark into the template itself as a built-in, always-on safeguard (equivalent to The Onion's/Faking News' own visible branding) — this is a design safeguard, not a content-judgment step, so it doesn't reintroduce human review.
...
- **"Mistaken for real news" risk is higher here than a plain text-only bot**, precisely because the templated graphic deliberately mimics real breaking-news visual language — this is the same trap The Fauxy has already fallen into (a feature for virality, a risk for misinformation complaints). The built-in "SATIRE" mark and fictional-only masthead are the mitigation, not a cure.
...
- **No live-news ingestion by design** — trades topical freshness for materially lower risk of satirizing a real, currently-newsworthy identifiable person.
